...
//       const shoeData = response.data.filter((data)=> data.detalis.subcategory === "shoes");
...
useEffect(() => {
    const fetchProducts = async () => {
      ...
      const shoeData = response.data.filter(
        (item) => item.details?.subcategory?.toLowerCase() === "shirts"
      );
  
      setProducts(shoeData); // Store filtered items
    };
